...
export function Inspiration() {
  ...
  // Process deviations to assign sizes for bento layout
    ...
    <div className="space-y-4">
        ...
        <div className="text-center py-12">
          {error instanceof ApiError && error.status === 429 ? (
            <>
              ...
              <p className="text-muted-foreground mb-2">DeviantArt API rate limit reached</p>
              <p className="text-sm text-muted-foreground/70 mb-4">
                Please wait a few minutes before trying again
              </p>
            </>
          ) : (
            <p className="text-muted-foreground mb-4">Failed to load deviations</p>
          )}
        ...
        <>
          ...
          <div ref={loadMoreRef} className="py-8 text-center">
            ...
            {!hasNextPage && processedDeviations.length > 0 && (
              <p className="text-muted-foreground text-sm">No more deviations</p>
            )}
          </div>
        </>
      )}
      ...
      {showBackToTop && (
        <Button
          variant="secondary"
          size="icon"
          className="fixed bottom-20 md:bottom-6 right-6 h-10 w-10 rounded-full shadow-lg z-50"
          onClick={scrollToTop}
        >
